...
public class GeradorPdfImplComIText implements GeradorEbook {
    ...
    public void gera(Ebook ebook) {
        ...
        try (var writer = new PdfWriter(Files.newOutputStream(arquivoDeSaida));
            ...
            for (Capitulo capitulo : ebook.getCapitulos()){
                ...
                List<IElement> convertToElements = HtmlConverter.convertToElements(capitulo.getConteudo());

                for (IElement element : convertToElements) {
                    pdfDocument.add((IBlockElement) element);
                }

                verificaUltimaPagina(ebook, pdfDocument, capitulo);
            }
        } catch (Exception ex) {
    ...
    private void verificaUltimaPagina(Ebook ebook, Document pdfDocument, Capitulo capitulo) {
